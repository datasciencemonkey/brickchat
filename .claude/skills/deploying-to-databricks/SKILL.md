...
databricks apps create brickchat --description "BrickChat - AI Chat Application"
```

Alternatively, create via the Databricks UI:
1. Navigate to **Compute** > **Apps** in your workspace
2. Click **Create App**
3. Name it `brickchat` and configure settings
...
| Create app (once) | `databricks apps create brickchat` |
| Deploy | `databricks apps deploy brickchat --source-code-path ./deployment` |
| Check status | `databricks apps get brickchat` |
| View logs | `databricks apps logs brickchat` |
| Health check | `curl http://localhost:8000/health` |
