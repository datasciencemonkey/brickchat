...
import 'autonomous_agents_page.dart';
...
class _SettingsPageState extends ConsumerState<SettingsPage> {
  ...
  Widget _buildAutonomousAgentsSection() {
    ...
    final enabledCount = agents.where((a) => a.isEnabled).length;

    return _buildNavigationCard(
      context,
      icon: Icons.smart_toy_outlined,
      title: 'Manage Autonomy',
      subtitle: agents.isEmpty
          ? 'Manage AI agents for autonomous routing'
          : '$enabledCount of ${agents.length} agent(s) enabled',
      onTap: () {
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) => const AutonomousAgentsPage(),
          ),
        );
      },
    );
